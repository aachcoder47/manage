-- Fix job applications table and relationships
-- This will fix the table name and add proper relationships

-- Step 1: Drop the old table if it exists
DROP TABLE IF EXISTS job_application CASCADE;

-- Step 2: Create the correct job_applications table with proper relationships
...
-- Step 3: Add foreign key constraints with proper names for Supabase
ALTER TABLE job_applications 
ADD CONSTRAINT job_applications_job_id_fkey 
FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE;

ALTER TABLE job_applications 
ADD CONSTRAINT job_applications_external_posting_id_fkey 
FOREIGN KEY (external_posting_id) REFERENCES external_job_posting(id) ON DELETE SET NULL;

-- Step 4: Create application_status_history table
...
-- Step 5: Add foreign key for status history
ALTER TABLE application_status_history 
ADD CONSTRAINT application_status_history_application_id_fkey 
FOREIGN KEY (application_id) REFERENCES job_applications(id) ON DELETE CASCADE;

-- Step 6: Create application_communications table
...
-- Step 7: Add foreign key for communications
...
-- Step 9: Create sample job applications for testing
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM jobs LIMIT 1) THEN
        ...
        RAISE NOTICE 'Created sample job applications for user_364WebGvJNOCngdeyz4qTP7wXXA';
    ELSE
        RAISE NOTICE 'No jobs found to create applications for';
    END IF;
END $$;

-- Step 10: Verify application creation
SELECT 
...
-- Step 11: Show sample applications with job details
SELECT 
