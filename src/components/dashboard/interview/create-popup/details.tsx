...
function DetailsPopup({
  ...
  const onGenrateQuestions = async () => {
    ...
    try {
      ...
      if (!generatedQuestions.data.response) {
        console.error("No response field in API data:", generatedQuestions.data);
      ...
      try {
        ...
        console.log("Raw response:", generatedQuestions.data.response);
        throw new Error("Failed to parse API response");
      }

      if (!generatedQuestionsResponse.questions || !Array.isArray(generatedQuestionsResponse.questions)) {
        console.error("No questions array in response:", generatedQuestionsResponse);
      ...
      const updatedQuestions = generatedQuestionsResponse.questions.map(
        (question: any, index: number) => {
          ...
          return {
            id: uuidv4(),
            question: question.question ? question.question.trim() : `Question ${index + 1}`,
            follow_up_count: 1,
          };
        },
      );
      ...
      console.log("Final interview data:", updatedInterviewData);
      
      // Set the interview data first, then the parent component will handle navigation
      setInterviewData(updatedInterviewData);
      
      // Reset the clicked state but don't set loading to false here
      // The parent component will handle the loading state change
      setIsClicked(false);
      ...
      if (axios.isAxiosError(error)) {
        ...
        alert(error instanceof Error ? error.message : "Error generating questions. Please try again.");
      }
  ...
  const onManual = () => {
    try {
      ...
      alert("Error setting up interview. Please try again.");
