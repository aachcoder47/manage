    ...
    const fetchAIData = async () => {
      ...
      try {
        ...
        if (assessments && assessments.length > 0) {
          const avg =
            assessments.map((a) => a.score || 0).reduce((a, b) => a + b, 0) / assessments.length;
  ...
  return (
    ...
    >
      <Card className="group relative h-72 w-full rounded-xl overflow-hidden border border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300">
        ...
        <CardContent
          className={`relative z-10 p-4 flex flex-col h-full justify-between ${isFetching ? "opacity-60" : ""}`}
        >
          ...
          <div className="flex items-end justify-between mt-auto">
            <div className="flex gap-2">
              <Button size="sm" className="bg-indigo-500/80 text-white" disabled={isFetching} onClick={handleCreateAssessment}>
                <Brain size={13} className="mr-1" />
                Assessments
              </Button>
              <Button size="sm" className="bg-purple-500/80 text-white" disabled={isFetching} onClick={handleFilterCandidates}>
                <Filter size={13} className="mr-1" />
                Filter Candidates
              </Button>
              <Button size="sm" className="bg-blue-500/80 text-white" disabled={isFetching} onClick={handleViewAnalytics}>
            ...
            <div>
              {img ? (
                <Image src={img} alt="Interviewer" width={42} height={42} className="rounded-full border-2 border-white/30" />
              ) : (
                <MiniLoader />
              )}
            </div>
